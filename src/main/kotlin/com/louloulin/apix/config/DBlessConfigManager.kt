...
class DBlessConfigManager(private val vertx: Vertx) {
    ...
    fun loadConfig(): Future<Void> {
        // 使用同步方式加载配置，避免使用executeBlocking导致的RejectedExecutionException
        try {
            ...
            if (Files.exists(configFile)) {
                ...
                // Save default configuration using synchronous method
                try {
                    val configFileToSave = Paths.get(configPath)
                    Files.createDirectories(configFileToSave.parent)
                    Files.writeString(configFileToSave, config.encodePrettily())
                    logger.info("Created default configuration at {}", configPath)
                } catch (e: Exception) {
                    logger.warn("Failed to save default configuration, but will continue with in-memory config", e)
                }
            }
    ...
    fun saveConfig(newConfig: JsonObject): Future<Void> {
        try {
            // Create backup of current configuration using synchronous method
            try {
                val configFile = Paths.get(configPath)
                if (Files.exists(configFile)) {
                ...
                logger.warn("Failed to create backup, but will continue with saving", e)
            }
    ...
    fun restoreFromBackup(version: Long): Future<Void> {
        try {
            ...
            if (Files.exists(backupFile)) {
                // Create backup of current configuration before restoring using synchronous method
                try {
                    val configFile = Paths.get(configPath)
                    if (Files.exists(configFile)) {
                        // Create backup filename with version
                        val backupFilePath = Paths.get(backupDir, "apix-config-v${configVersion.get()}.json")
                        // Create backup directory if it doesn't exist
                        Files.createDirectories(Paths.get(backupDir))
                        // Copy current configuration to backup
                        Files.copy(configFile, backupFilePath, StandardCopyOption.REPLACE_EXISTING)
                        logger.info("Created backup of configuration at {}", backupFilePath)
                    }
                } catch (e: Exception) {
                    logger.warn("Failed to create backup before restore, but will continue with restore", e)
                }
                ...
                // Reload configuration using synchronous method
                try {
                    val configContent = Files.readString(configFile)
                    ...
                    logger.error("Failed to reload configuration after restore", e)
                    return Future.failedFuture(e)
                }
            } else {
    ...
    fun listBackupVersions(): Future<List<Long>> {
        try {
            ...
            if (Files.exists(backupPath)) {
                ...
                return Future.succeededFuture(backupFiles)
            } else {
    ...
    fun applyPartialUpdate(update: JsonObject): Future<Void> {
        try {
            ...
            // Save the updated configuration using synchronous method
            try {
                ...
                logger.info("Applied partial update to configuration, version: {}", configVersion.get())

                return Future.succeededFuture()
            } catch (e: Exception) {
                logger.error("Failed to save configuration after partial update", e)
                return Future.failedFuture(e)
            }
        } catch (e: Exception) {
