...
class SimpleEmbeddingEngine(private val vertx: Vertx) : EmbeddingEngine {
    ...
    private fun computeEmbedding(text: String): FloatArray {
        ...
        // 定义关键词集合
        val codeKeywords = setOf("code", "program", "algorithm", "function", "class", "method",
                              "java", "python", "javascript", "kotlin", "programming", "developer",
                              "software", "implementation", "code", "coding")

        val creativeKeywords = setOf("creative", "story", "poem", "novel", "write", "writing",
                                  "poetry", "fiction", "narrative", "creative", "imagination",
                                  "artistic", "literature", "author", "poet", "writer")

        // 计算词频和权重
        val wordFreq = mutableMapOf<String, Float>()
        words.forEach { word ->
            // 基础权重
            var weight = 1.0f
            ...
            if (codeKeywords.contains(word)) {
                weight = 5.0f  // 代码相关关键词权重更高
            } else if (creativeKeywords.contains(word)) {
                weight = 5.0f  // 创意写作相关关键词权重更高
            }

            wordFreq[word] = wordFreq.getOrDefault(word, 0f) + weight
        }

        // 创建嵌入向量（使用词的哈希值作为索引）
        val embedding = FloatArray(vocabularySize) { 0f }

        wordFreq.forEach { (word, weight) ->
