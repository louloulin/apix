...
class EdgeControlVerticle : BaseVerticle() {
    ...
    override fun registerEventBusHandlers() {
        ...
        vertx.eventBus().consumer<JsonObject>(EventBusAddresses.EDGE_NODE_DETAILS_GET) { message ->
            ...
            edgeControlCenter.getNodeDetail(nodeId)
        ...
        vertx.eventBus().consumer<JsonObject>(EventBusAddresses.EDGE_NODES_BATCH_OPERATE) { message ->
            ...
            edgeControlCenter.batchOperateNodes(operation, nodeIds, params)
                .onSuccess { result ->
                    sendSuccess(message, result)
                }
                .onFailure { cause ->
        ...
        vertx.eventBus().consumer<JsonObject>(EventBusAddresses.EDGE_NODE_ALERTS_GET) { message ->
            ...
            val severity = filter.getString("severity", "")
            val status = filter.getString("status", "")
            val page = filter.getInteger("page", 1)
            val pageSize = filter.getInteger("pageSize", 10)
            edgeControlCenter.getNodeAlerts(nodeId, severity, status, page, pageSize)
                .onSuccess { alerts ->
        ...
        vertx.eventBus().consumer<JsonObject>(EventBusAddresses.EDGE_NODE_DIAGNOSE) { message ->
            ...
            edgeControlCenter.diagnoseNode(nodeId, diagnosticType, params)
                .onSuccess { result ->
                    sendSuccess(message, result)
                }
                .onFailure { cause ->
        ...
        vertx.eventBus().consumer<JsonObject>(EventBusAddresses.EDGE_NODE_UPDATE_SOFTWARE) { message ->
            ...
            val nodeIds = listOf(nodeId)
            edgeControlCenter.updateNodeSoftware(nodeIds, updateType, updateInfo.getString("version", ""), updateInfo)
                .onSuccess { result ->
                    sendSuccess(message, result)
                }
                .onFailure { cause ->
