...
export const options = {
  // 阶段式负载配置 - 更激进的负载模式
  stages: [
    { duration: '10s', target: 1000 },   // 快速增加到 1000 个并发用户
    { duration: '20s', target: 5000 },   // 增加到 5000 个并发用户
    { duration: '30s', target: 10000 },  // 增加到 10000 个并发用户
    { duration: '60s', target: 20000 },  // 增加到 20000 个并发用户
    { duration: '60s', target: 50000 },  // 增加到 50000 个并发用户
    { duration: '30s', target: 10000 },  // 减少到 10000 个并发用户
...
export default function() {
  ...
  const success = check(response, {
    ...
    'response body contains Hello': (r) => r.body && r.body.includes('Hello'),
  });
...
export function handleSummary(data) {
  ...
  return {
    ...
    './k6-high-load-results.json': JSON.stringify(data),
  };
}
