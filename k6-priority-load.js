...
export const options = {
  ...
  batchPerHost: 10,             // 每个主机的批处理请求数
  
  // 阶段式负载配置 - 更合理的负载增长
  stages: [
    { duration: '30s', target: 1000 },    // 逐步增加到 1000 个并发用户
    { duration: '30s', target: 5000 },    // 增加到 5000 个并发用户
    { duration: '60s', target: 10000 },   // 增加到 10000 个并发用户
    { duration: '60s', target: 20000 },   // 增加到 20000 个并发用户
    { duration: '120s', target: 50000 },  // 增加到 50000 个并发用户
    { duration: '60s', target: 20000 },   // 减少到 20000 个并发用户
...
export function handleSummary(data) {
  ...
  return {
    ...
    './k6-priority-load-results.json': JSON.stringify(data),
  };
}
